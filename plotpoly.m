...
edgeh = zeros(n,1);
lblh = NaN*zeros(n,2);
edgeh(1) = plot(real(w(1:2)),imag(w(1:2)),'-','linewidth',lw);
ang = angle(w(2)-w(1));
hold on
axis(lim)
...
while j <= n
  ...
  if ~atinf(jp1)
    ...
    edgeh(j) = plot(real(w([j jp1])),imag(w([j jp1])),'-','linewidth',lw);
    ang = ang - pi*beta(j);
    j = j+1;
  else
    ...
    edgeh(j) = plot(real(z),imag(z),'-','linewidth',lw);
    ...
    edgeh(j+1) = plot(real(z),imag(z),'-','linewidth',lw);
    if nargin == 3
      theta = ang + pi;
