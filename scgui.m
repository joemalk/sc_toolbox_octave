...
%   ported to Octave 5.0 by Joseph Malkoun
...
fig = figure('name','Schwarz-Christoffel Mapping','numbertitle','off',...
    'tag','scgui','menubar','none','visible','off');
set(fig,'defaultuicontrolinterruptible','on')
...
ss = get(0,'screensize');
set(0,'units',unit)
optsize = factor*0.65*ss(3:4);
truesize = max(optsize,minsize);
set(fig,'units','characters','position',[0 factor*ss(4)-truesize(2)-8 truesize])
...
% Create axes
ax(1) = axes('units','characters','tag','PhysicalAxes');
ax(2) = axes('units','characters','tag','CanonicalAxes');
set(ax,'nextplot','add','box','on','plotboxaspectratio',[1 1 1])
...
if changed
  ...
  set(findobj(data.SCfig,'tag','MeshLines'),'erase','norm','visible','off')
...
switch class(data.map)
  case { 'diskmap','extermap','crdiskmap' }
    h = plot(exp(i*linspace(0,2*pi,100)),'linewidth',1);
    axis normal
    axis equal
    axis square
  ...
  case 'stripmap'
    ...
    d = (maxx-minx)/2;
    m = (maxx+minx)/2;
    h(1) = plot([m-1.2*d,m+1.2*d],[0,0],'linewidth',1);
    h(2) = plot([m-1.2*d,m+1.2*d],[1,1],'linewidth',1);
    axis normal
    axis equal
...
set(findobj(ax(1)),'visible','on')
set(findobj(ax(2)),'visible','off')
set(ax,'buttondown','')
...
if ~data.iscurrent
  ...
  f = figure('units','characters','visible','off','integerhandle','off',...
      'name','Map parameters','numbertitle','off','position',pos);
  u = uicontrol('style','text','units','characters',...
      'position',pos,'fontname','FixedWidth','horizontalalignment','left',...
...
set(hp,'tag','PolygonPlot','uicontextmenu',data.PolygonContextMenu)
...
set(fig2,'visible','on')
uiwait(fig2)
set(fig2,'visible','off')			% turn it off
...
if or(viewdom==1 , viewdom==2)
  set(ax,'position',window)
else
  % They must occupy the window together
  dx = .46*window(3);
  dy = window(4);
  set(ax(1),'position',[window(1) window(2) dx dy])
  set(ax(2),'position',[window(1)+window(3)-dx window(2) dx dy])
end
...
if ~isempty(data) & data.iscurrent
  set(findobj(ax(vis)),'buttondownfcn',@addpt)
else
  set(findobj(ax(vis)),'buttondownfcn','')
end
set(findobj(ax(~vis)),'buttondownfcn','')
...
for l = 1:length(idx)
  ...
  tmp = gca;
  axun = get(tmp,'units');  axpos = get(tmp,'position');
  newax = copyobj( ax(idx(l)), newfig );
  delete(tmp)
  set(newax,'units',axun,'position',axpos);
...
set(h, 'tag','PolygonPlot', 'uicontextmenu',data.PolygonContextMenu);
...
set(findobj([data.PhysicalAxes,data.CanonicalAxes]),'buttondownfcn','')
...
fig2 = figure('visible','off','number','off','menu','none','name',name);
pos = get(fig2,'position');
height = 150-34*(nvar==1);
set(fig2,'position',[pos(1),pos(2),400,height])
uicontrol(fig2,'style','frame','position',[0,0,400,height]);
for j=1:nvar
  height = 64+34*(nvar-j);
  eval(sprintf('xlab=xlab%d;x=x%d(:);',j,j))
  uicontrol(fig2,'style','text','position',[10,height,80,24],...
      'string',xlab,'hor','r');
  xstr = ['[ ',sprintf(' %.4g%+.4gi ',[real(x)';imag(x)']),']'];
  ui(j) = uicontrol(fig2,'style','edit','position',[100,height,290,24],...
      'string',xstr,'tag','scgeditbox');
end
uicontrol(fig2,'style','push','position',[40,20,60,24],...
    'string','Clear','callback',...
    'set(findobj(gcf,''tag'',''scgeditbox''),''string'',''[ ]'')')
uicontrol(fig2,'style','push','position',[170,20,60,24],...
    'string','Done','callback','global SC_FLAG,SC_FLAG=1;')
uicontrol(fig2,'style','push','position',[300,20,60,24],...
    'string','Cancel','callback','global SC_FLAG,SC_FLAG=-1;')
set(fig2,'visible','on')
global SC_FLAG
SC_FLAG = 0;
while ~SC_FLAG
...
for j=1:length(names)
  icons{j} = im2double(imread(['private/' names{j} '.png']));
end
action(1) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{1},...
    'tooltipstring','Draw new polygon',...
    'units','pixels','position',[0 0 42 42],...
    'Callback',@draw,...
    'tag','DrawButton');
% Determine how large the icons are in character units on this computer. 
set(action(1),'units','characters')
icon_size = get(action(1),'position');
icon_size = icon_size(3:4);
action(2) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{2},...
    'tooltipstring','Modify current polygon',...
    'Callback',@domodify,...
    'tag','ModifyButton');
action(3) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{3},...
    'tooltipstring','Solve for parameters',...
    'Callback',@solve,...
    'tag','SolveButton');
action(4) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{4},...
    'tooltipstring','Set conformal center',...
    'Callback',@recenter,...
    'tag','CenterButton');
action(5) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{5},...
    'tooltipstring','Plot a mesh',...
    'Callback',@meshplot,...
    'tag','PlotButton');
action(6) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{6},...
    'tooltipstring','Inspect map details',...
    'Callback',@scdisplay,...
    'tag','ShowButton');
action(7) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{7},...
    'tooltipstring','Import/export to command line',...
    'Callback',@importexport,...
    'tag','ImportExportButton');
action(8) = uicontrol('parent',fig,'style','push',...
    'cdata',icons{8},...
    'tooltipstring','Quit SC GUI',...
    'Callback',@scquit,...
    'tag','QuitButton');

% CONTEXT MENUS
cm = uicontextmenu('tag','PolygonContextMenu');
uimenu(cm,'label','Modify...','callback',@domodify)
uimenu(cm,'label','Inspect vertices...','callback',@edit)
uimenu(cm,'label','Delete...','callback',@menudeletepoly)
uimenu(cm,'label','Cancel action','separator','on')

cm = uicontextmenu('tag','PointContextMenu');
uimenu(cm,'label','Inspect...','callback',@editpts)
uimenu(cm,'label','Delete...','callback',@deletepoints)
uimenu(cm,'label','Cancel action','separator','on')

% FIGURE MENUS
filemenu = uimenu(fig,'label','File');
uimenu(filemenu,'label','Save map data...','callback',@savedata,...
       'tag','FileSaveMenu');
uimenu(filemenu,'label','Load map data...','callback',@loaddata,...
       'tag','FileLoadMenu');
uimenu(filemenu,'label','Import/export to workspace...',...
       'callback',@importexport,'tag','FileImportExportMenu');
uimenu(filemenu,'label','Make a printable copy','callback',@printablecopy);
uimenu(filemenu,'label','Quit SC mapping...','callback',@scquit);
polymenu = uimenu(fig,'label','Polygon');
uimenu(polymenu,'label','New...','callback',@draw,'tag','PolyNewMenu');
uimenu(polymenu,'label','Modify...','callback',@domodify,'tag','PolyModifyMenu');
uimenu(polymenu,'label','Edit numerically...','callback',@edit,...
       'tag','PolyEditMenu');
mapmenu = uimenu(fig,'label','Map');
uimenu(mapmenu,'label','Solve for parameters','callback',@solve,...
       'tag','MapSolveMenu');
uimenu(mapmenu,'label','Reset conformal center...','callback',@recenter,...
       'tag','MapCenterMenu');
uimenu(mapmenu,'label','Plot a mesh','callback',@meshplot,...
       'tag','MapPlotMenu');
uimenu(mapmenu,'label','Inspect details','callback',@scdisplay,...
       'tag','MapShowMenu');
toolsmenu = uimenu(fig,'label','Tools');
tutmenu = uimenu(toolsmenu,'label','Tutorials','separator','on');
uimenu(tutmenu,'label','Basic','callback','scdtutor');
uimenu(tutmenu,'label','Infinite vertices','callback','scdinf');
uimenu(tutmenu,'label','Elongated polygons','callback','scdlong');
uimenu(tutmenu,'label','Faber polynomials','callback','scdfaber');
uimenu(toolsmenu,'label','Laplace solver...','separator','on',...
       'callback',@lapsolver,'tag','ToolsLaplaceMenu');
...
set(data.SettingsFrame,'position',[figpos(3)-23 0 23 h])
set(data.ActionsFrame,'position',[0 h figpos(3) data.iconsize(2)+0.5]);
...
w = figpos(3) - 22;
set(data.TolText, 'position',[w+1 h+23.5 20 1.25])
set(data.TolEdit, 'position',[w+6.5 h+21.5 9 1.5])
%set(data.TraceBox, 'position',[w+1 h+18.75 20 1.75])
set(data.DomainText, 'position',[w+1 h+15.5 20 1.25])
set(data.DomainPopup, 'position',[w+1 h+13.25 20 1.75])
set(data.ViewText, 'position',[w+1 h+10 20 1.25])
set(data.ViewPopup, 'position',[w+1 h+7.75 20 1.75])
set(data.MeshText, 'position',[w+1 h+4.5 20 1.25])
set(data.XRText, 'position',[w+1 h+2.75 7 1.25])
set(data.XREdit, 'position',[w+1 h+1 20 1.5])
set(data.YTText, 'position',[w+1 h-1.25 7 1.25])
set(data.YTEdit, 'position',[w+1 h-3 20 1.5])

% ACTIONS
h = figpos(4) - data.iconsize(2) - 0.25;
dx = data.iconsize(1);
dy = data.iconsize(2);
set(data.DrawButton, 'position',[1 h dx dy])
set(data.ModifyButton, 'position',[1+dx h dx dy])
set(data.SolveButton, 'position',[4+2*dx h dx dy])
set(data.CenterButton, 'position',[4+3*dx h dx dy])
set(data.PlotButton, 'position',[7+4*dx h dx dy])
set(data.ShowButton, 'position',[7+5*dx h dx dy])
set(data.ImportExportButton, 'position',[10+6*dx h dx dy])
set(data.QuitButton, 'position',[13+7*dx h dx dy])
...
fig = figure('visible','off','number','off','name','Import/Export',...
    'tag','sc_importexport','handlevisibility','callback',...
    'units','characters','menu','none','integerhandle','off','resize','off');

set(fig,'position',[0,0,41,10])
movegui(fig,'center');

set(fig,'defaultuicontrolunits','characters')

uicontrol(fig,'style','frame','position',[0,0,41,10])
