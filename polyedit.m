...
if isempty(fig) 
  ...
  ss = get(0,'screensize');
  set(0,'units',un)
  siz = max( factor*0.65*ss(3:4), [95 36] );
  set(fig,'units','characters','position',[factor*ss(3:4)-siz-[8 4] siz])
  set(fig,'closerequestfcn',@PEQuit)
else
  % Save current state.
  turn_off_hold = ~ishold;
  figprops = {'units';'position';'resizefcn';'pointer';'windowbuttondownfcn';...
	'windowbuttonupfcn';'pointer';'menubar';'name';'numbertitle'};
  figstate = get(fig,figprops);
  axprops = {'units';'position';'xgrid';'ygrid'};
...
if ~isempty(poly)
  ...
  for j = 1:length(w)
	...
	'markerfacecolor','b');
    if isinf(w(j))
      set(data.Vertices,'userdata',[ x{j} y{j} ])
    else
      set(data.Vertices,'userdata',[ x{j}(1) y{j}(1) ] )
    end
  end
  data.addmode = 'normal';
  guidata(fig,data)
...
set(data.Figure,'windowbuttondownfcn',@PEAddStart)
set(data.Vertices,'buttondownfcn','','hittest','off','erasemode','norm')
set(data.Edges,'buttondownfcn','','hittest','off')
...
set(data.Vertices,'buttondownfcn','','hittest','off','erasemode','norm')
set(data.Edges,'buttondownfcn',@PEInsert,'hittest','on')
...
set(data.Figure,'windowbuttondownfcn','')
set(data.Vertices,'buttondownfcn',@PEMoveStart,'hittest','on','erasemode','norm')
set(data.Edges,'buttondownfcn','','hittest','off')
...
set(data.Figure,'windowbuttondownfcn','')
set(data.Vertices,'buttondownfcn',@PEDelete,'hittest','on')
set(data.Edges,'buttondownfcn','','hittest','off')
...
data = guidata(obj);
if and(get(data.SnapToggle,'value') , strcmp(get(data.SnapToggle,'enable'),'on'))
...
data = guidata(obj);
if and(get(data.DiscAngToggle,'value') , ...
...
data = guidata(obj);
if and(get(data.DiscLenToggle,'value') , ...
...
data = guidata(obj);
%%set(data.PreviewLine,'visible','on')
set(data.Figure,'windowbuttonmotionfcn',@PEAddMove)
set(data.Figure,'windowbuttonupfcn',@PEAddFinish)
PEAddMove(obj)
...
if ~isnan(P(1))
  ...
  switch data.addmode
	...
	data.Vertices(n+1) = plot(P(1),P(2),'o','userdata',[P(1) P(2)]);
      else
	data.atinf(m+1) = 1;
	...
	data.polybeta(n+1) = NaN;
	data.Vertices(n+1) = plot(Inf,Inf,'o','userdata',[P(1) P(2)]);
      end
      if n > 0
	data.Edges(n) = plot(x(m:m+1),y(m:m+1),'-','erasemode','norm');
	set(data.Edges(n),'userdata',n)
      end
    case 'infinite'
      data.atinf(m+1) = 1;
      set(data.Figure,'pointer','crosshair')
      data.addmode = 'return';
      pt = get(data.Vertices(n+0.5),'userdata');
      set(data.Vertices(n+0.5),'userdata',[pt;[P(1) P(2)]])
    case 'return'
      data.Edges(n) = plot(x(m:m+1),y(m:m+1),'-','erasemode','norm');
      set(data.Edges(n),'userdata',n)
      data.atinf(m+1) = 0;
...
set(data.Figure,'windowbuttonmotionfcn',@PEMoveMove)
set(data.Figure,'windowbuttonupfcn',@PEMoveFinish)
...
k = data.moveselected;
set(data.Vertices(k),'xdata',xy(1),'ydata',xy(2),'userdata',xy)
...
if ~isnan(adj(1))
  ...
  if isinf(w(j))
    ...
    pts = get(data.Vertices(j),'userdata');
    pts(2,:) = [real(y(1)) imag(y(1))];
    set(data.Vertices(j),'userdata',pts)
  else
    y = [w(j),xy(1)+i*xy(2)];
...
newVertex = plot(real(P),imag(P),'bo',...
    'markerfacecolor','b','userdata',[real(P) imag(P)]);
...
succ = plot([real(P) x(2)],[imag(P) y(2)],'-','erasemode','norm');
...
if ~data.isclosed & ( k==1 | k==n ) 
  ...
  elseif infp & ~infs
    ...
    pts = get(data.Vertices(pred),'userdata');
    pts(2,:) = [real(side(1)) imag(side(1))];
    set(data.Vertices(pred),'userdata',pts)
  elseif ~infp & infs
    axlim = axis;
    scale = 2*max( axlim(2)-axlim(1), axlim(4)-axlim(3) );
    beta(succ) = beta(succ) + beta(k);
    side = w(pred) + [0 scale*sign(w(k)-w(pred))];
    pts = get(data.Vertices(succ),'userdata');
    pts(1,:) = [real(side(2)) imag(side(2))];
    set(data.Vertices(succ),'userdata',pts)
  else
    errordlg('To be deleted, a vertex must have at least one finite neighbor.','PolyEdit Error')
    return
  end
...
if n > 1
  data.Edges(n) = plot(x([m,1]),y([m,1]),'-','userdata',n);
  if n==2
...
pts = get(data.Vertices,'userdata');
if isempty(pts)
  pts = zeros(0,2);
elseif size(pts,1)>1
  pts = cat(1,pts{:});
...
set(fig,'units','characters','defaultuicontrolunits','characters')
...
widget(1) = uicontrol('style','togglebutton',...
    'cdata',icons{1},...
    'tooltipstring','Add vertices in sequence',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEAddMode,...
    'tag','AddButton');
widget(end+1) = uicontrol('style','togglebutton',...
    'cdata',icons{2},...
    'tooltipstring','Insert vertices',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEInsertMode,...
    'tag','InsertButton');
widget(end+1) = uicontrol('style','togglebutton',...
    'cdata',icons{3},...
    'tooltipstring','Move vertices',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEMoveMode,...
    'tag','MoveButton');
widget(end+1) = uicontrol('style','togglebutton',...
    'cdata',icons{4},...
    'tooltipstring','Delete vertices',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEDeleteMode,...
    'tag','DeleteButton');
widget(end+1) = uicontrol('style','push',...
    'cdata',icons{5},...
    'tooltipstring','Start over',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEClear,...
    'tag','ClearButton');
...
widget(end+1) = uicontrol('style','push',...
    'cdata',icons{8},...
    'tooltipstring','Discard and quit',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEQuit,...
    'tag','CancelButton');
...
widget(end+1) = uicontrol('style','togglebutton',...
    'cdata',icons{2},...
    'tooltipstring','Discretize length',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEDiscLenMode,...
    'tag','DiscLenToggle');
widget(end+1) = uicontrol('style','togglebutton',...
    'cdata',icons{3},...
    'tooltipstring','Discretize angle',...
    'units','pixels',...
    'position',[0 0 42 42],...
    'callback',@PEDiscAngMode,...
    'tag','DiscAngToggle');
...
set(gca,'units','characters','box','on','xgrid','off','ygrid','off',...
...
set(widget,'units','characters')
icon_size = get(widget(1),'position');
...
set(data.Axes,'position',[ 8 5 figpos(3)-39 figpos(4)-h]);
...
wb = (21-is(1))/2;
h = figpos(4) - 2 - 2*is(2);
set(data.SnapToggle, 'position',[w+wb h is(1) is(2)])
h = h - 1.75;
set(data.SpacingText, 'position',[w+1 h 10 1.25])
set(data.GridSpaceEdit, 'position',[w+11.5 h 9.5 1.5])
h = h - 2.5 - is(2);
set(data.DiscLenToggle, 'position',[w+wb h is] )
h = h - 1.75;
set(data.LenQuantText, 'position',[w+1 h 11 1.25])
set(data.LenQuantEdit, 'position',[w+13 h 6 1.5])
h = h - 2.5 - is(2);
set(data.DiscAngToggle, 'position',[w+wb h is] )
h = h - 1.75;
set(data.AngQuantText, 'position',[w+1 h 10 1.25])
set(data.PiText, 'position',[w+11.5 h 4 1.25])
set(data.AngQuantEdit, 'position',[w+16 h 5 1.5])
h = h - 4;
set(data.XLimText, 'position',[w+1 h 9 1.25])
set(data.XLimEdit, 'position',[w+11 h 10 1.5])
h = h - 2.5;
set(data.YLimText, 'position',[w+1 h 9 1.25])
set(data.YLimEdit, 'position',[w+11 h 10 1.5])

% Actions
h = figpos(4) - data.iconsize(2) - 0.25;
dx = data.iconsize(1);
dy = data.iconsize(2);
set(data.AddButton, 'position',[1 h dx dy])
set(data.MoveButton, 'position',[1+dx h dx dy])
set(data.InsertButton, 'position',[1+2*dx h dx dy])
set(data.DeleteButton, 'position',[1+3*dx h dx dy])
set(data.ClearButton, 'position',[4+4*dx h dx dy])
set(data.FinishButton, 'position',[4+5*dx h dx dy])
set(data.AcceptButton, 'position',[7+6*dx h dx dy])
set(data.CancelButton, 'position',[7+7*dx h dx dy])
